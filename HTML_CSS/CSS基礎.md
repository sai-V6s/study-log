...
・CSSは見た目を制御するためのもので、HTMLなどで構造を定義した要素に
　色やフォント、スペースなどのスタイルを指定できる。
　CSSはWebぺ時を一貫して適用できるので、ページを跨いでもデザインを継承できる
・CSSの構成
　CSSは、「セレクタ」と「プロパティ」を組み合わせて構成されている
　言葉は横文字でむずかしいが、セレクタとは要素で、その要素に対してのデザインをプロパティと言っている
